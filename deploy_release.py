...
class run_process_genfile():
    ...
    def create_jsonfile(self, df, mvp):
        ...
        df['Sub_Git_Path'] = df['Git_Path'].str[26:29]
        df['name_file'] = df['Sub_Git_Path'].apply(condition)
    ...
    def create_textfile(self, df_sheet, df_ddl):
        for mvp in self.str_mvp:
            if (mvp == 'MVP1') and df_sheet:
                # sheet
                if 'MVP1' in df_sheet.keys():
