CREATE OR REPLACE VIEW P1VEULQ.VLQ_07_GL_BAL_SUM_RC_D (
  SET_OF_BOOKS_ID,
  RC_CODE,
  PROD_CODE,
  AC_CODE,
  ...
  NOW_BALANCE_BEQ)
AS SELECT 
CAST(coab.SET_OF_Books_CD  AS DECIMAL (15))        SET_OF_BOOKS_ID                             
,RTRIM(CAST(coa.RC_Val  AS CHAR (4)))           RC_CODE                                           
...
,CASE 
        ...
        THEN CAST('CR' AS VARCHAR (2))                             
        ELSE CAST( 'DR' AS VARCHAR (2))                                      
END  NATURE_NOW                           
,SUM(CASE 
        WHEN coa.chart_of_account_desc IN ('L','O','R') 
